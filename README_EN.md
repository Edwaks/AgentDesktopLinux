...
**AgentDesktopLinux** is the TaskCapitan agent layer for desktop platforms. It is designed for TaskCapitan users and platform-task operators and connects TaskCapitan assignments with actions on a supported workstation platform.

- Public URL: https://taskcapitan.ru/
- Core entities: assignments, platforms, sessions, steps, confirmations, and results.
- Key capabilities: receiving assignments, platform context, step-by-step execution, action confirmation, returning results, session history, status control.

## Why the Product Is Needed

A real workflow rarely consists of a single action. Users must understand the initial situation, gather information, choose the next step, coordinate a decision, and return to the result later. AgentDesktopLinux provides one domain-specific environment for this work. It preserves context between stages and shows not only an individual item, but also its place in the overall process.

The product’s main value is not the number of screens, but workflow continuity. Assignments, platforms, sessions, steps, confirmations, and results are treated as parts of one user journey. New participants understand the current state faster, experienced specialists spend less time matching information manually, and managers gain a clear overview without requiring every step to be retold.
...
The roles Requester, Operator, and Result reviewer work with the same domain model from different perspectives. Consistent names for objects and states reduce ambiguity during handoffs. A responsible participant can reference a specific object and expected result instead of repeating the entire context.

## Main Sections

The public description covers these user capabilities:

- **Receiving assignments** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Platform context** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Step-by-step execution** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Action confirmation** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Returning results** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Session history** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Status control** — a distinct part of the workflow, connected to the product model and a clear user outcome.
